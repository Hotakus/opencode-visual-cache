...
- **Bottom Status Bar**: single-line hit rate (with trend) · Tokens · Balance in the prompt hint row — visible even with the sidebar closed; hide it anytime via `/cache-section` (disabling it also yields the prompt slot; requires a TUI restart)
...
The plugin supports slash commands and command palette (`Ctrl + P`) for runtime configuration. Changes take effect immediately and are persisted (the **Bottom Bar** toggle is the one exception — see [4.3 Section Visibility](#43-section-visibility)):
...
| `/cache-section` | Toggle sections & border | Independently show/hide Detail, Model & Pricing, Token Distribution, Loaded Skills, Balance, Bottom Bar, or the panel border (the Bottom Bar toggle requires a TUI restart) |
...
> **The Bottom Bar toggle requires a TUI restart**: on opencode 1.x the bottom bar is implemented by rebuilding the host's `session_prompt` slot, and that slot uses replace mode — **every rebuilder's output is rendered side by side**. If another plugin rebuilds `session_prompt` as well (e.g. [`oh-my-openagent`](https://github.com/code-yeongyu/oh-my-openagent)), you get **duplicate input boxes**.
>
> Run `/cache-section` to turn the **Bottom Bar** off and **restart the TUI**; this plugin then yields the slot and the conflict disappears (the bar is gone and the hint row shows the host's default path). opencode 2.x uses a dedicated `prompt.footer.status` slot, so there is no such conflict and no configuration is needed.
